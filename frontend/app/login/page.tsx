  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      const res = await apiFetch('/auth/mock/login', undefined, {
        method: 'POST',
        body: JSON.stringify({ email, org_id: orgId, mfa }),
      });
      const data = await res.json();
      localStorage.setItem('audity_token', data.access_token);
      if (data.refresh_token) {
        localStorage.setItem('audity_refresh_token', data.refresh_token);
      }
      ...
      <div className="card">
        ...
        <form onSubmit={onSubmit}>
          ...
          <div style={{ marginTop: 12 }}>
            <label>
              <input type="checkbox" checked={mfa} onChange={(e) => setMfa(e.target.checked)} /> Include MFA claim
            </label>
          </div>
          <div style={{ marginTop: 12 }}>
